...
public class CobarSqlMapClientTemplateWithSqlActionShardingRouterTest extends AbstractTestNGCobarClientTest {

    private transient final Logger logger = LoggerFactory.getLogger(CobarSqlMapClientTemplateWithSqlActionShardingRouterTest.class);
    ...
    public void testDeleteOnCobarSqlMapClientTemplate() {
        ...
        for (String name : names) {
            int affectedCount = getSqlMapClientTemplate().delete("com.alibaba.cobar.client.entities.Follower.deleteByName", name);
            if (name.startsWith("A")) {
    ...
    public void testQueryForListOnCobarSqlMapClientTemplateWithPartialDataOnDefaultDataSource() {
        ...
        List<Follower> followers = (List<Follower>) getSqlMapClientTemplate().queryForList("com.alibaba.cobar.client.entities.Follower.findAll");
        assertTrue(CollectionUtils.isNotEmpty(followers));
        ...
        followers = null;
        followers = (List<Follower>) getSqlMapClientTemplate().queryForList("com.alibaba.cobar.client.entities.Follower.finaByNameAlike", "A");
        assertTrue(CollectionUtils.isNotEmpty(followers));
        // only followers with name that starts with "A" can be found
        assertEquals(3, followers.size());
        for (Follower follower : followers) {
        ...
        followers = null;
        followers = (List<Follower>) getSqlMapClientTemplate().queryForList("com.alibaba.cobar.client.entities.Follower.finaByNameAlike", "D");
        assertTrue(CollectionUtils.isEmpty(followers));

        followers = null;
        followers = (List<Follower>) getSqlMapClientTemplate().queryForList("com.alibaba.cobar.client.entities.Follower.finaByNameAlike", "S");
        assertTrue(CollectionUtils.isEmpty(followers));
    }
    ...
    public void testQueryForObjectOnCobarSqlMapClientTemplate() {
        ...
        Follower follower = new Follower(name);
        Object pk = getSqlMapClientTemplate().insert("com.alibaba.cobar.client.entities.Follower.create", follower);
        if (pk != null) {
            Follower f = (Follower) getSqlMapClientTemplate().queryForObject("com.alibaba.cobar.client.entities.Follower.finaByName", name);
            assertNotNull(f);
